---
layout: page
---

[
[Home](./index.html) | 
[Schedule](./schedule.html) |
[Policy](./policy.html)
]

# Schedule 
Lecture: MW 3:30-4:50pm (Aug 24 -- Dec 7) Location: JAVITS LECTR 109 WESTCAMPUS

Office hour: MW 1-2pm NCS 351 (If you want to lock in a time, you can [book a meeting with GCal](https://calendar.app.google/5fEbJJZW86Gcgije6))

<!-- Office hour: T 3-5pm in this [Zoom meeting room](https://stonybrook.zoom.us/j/94704333277?pwd=NFhrbGYzUW50ZSt5R0Z3d3hobk54Zz09). Please book a meeting before 6pm using this [link](https://calendly.com/shuaimu/officehour). Each slot is 15 minutes. You can book 2 slots for 30 minutes, 3 slots for 45 minutes, etc. -->


<!-- <iframe src="https://docs.google.com/spreadsheets/d/e/2PACX-1vRAGmy_JuMcY5_Jv4z4y8H6WTwPWlb1Rq72sVI2ARAFV7nYetQh0tPJZWX_NsB7gqbgjpLvulzYdY2U/pubhtml?gid=0&amp;single=true&amp;widget=true&amp;headers=false"></iframe> -->

The schedule is tentative and subject to change. Each session includes one student presentation; topics TBA.

<style>
table td:first-child {
    vertical-align: top;
}
</style>

| Date &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;  | Topic &nbsp;                    |
|--------|---------------------------------------------------------------------|
|        | Session I: Welcome and Architecture                                             |
| Aug 24 | Overview<br>[How to Read a Paper](readings/paper-reading.pdf) |
| Aug 26 | Student presentation |
| Aug 31 | Architecture II<br>[TiDB: A Raft-based HTAP Database](readings/tidb.pdf) |
|        | Session II: Replication                                             |
| Sep 02 | Raft<br>[In Search of an Understandable Consensus Algorithm](readings/raft.pdf) |
| Sep 07 | No class (Labor Day)                                                |
| Sep 09 | MongoDB Replication<br>[Fault-Tolerant Replication with Pull-Based Consensus in MongoDB](readings/mongodb-nsdi.pdf) |
| Sep 14 | Student presentation |
|        | Session III: Index and Storage                                             |
| Sep 16 | Student presentation |
| Sep 21 | LSM-Tree<br>[The Log-Structured Merge-Tree (LSM-Tree)](readings/lsm.pdf) |
| Sep 23 | RocksDB<br>[Evolution of Development Priorities in Key-value Stores Serving Large-scale Applications: The RocksDB Experience](readings/rocksdb.pdf) |
| Sep 28 | **Exam 1** |
|        | Session IV: B+ Trees |
| Sep 30 | B+ Tree<br>[Efficient Locking for Concurrent Operations on B-Trees](readings/lehman-yao.pdf) |
| Oct 05 | Masstree<br>[Cache Craftiness for Fast Multicore Key-Value Storage](readings/masstree.pdf) |
| Oct 07 | Student presentation |
| Oct 12 | No class (Fall break) |
| Oct 14 | Project lightning talks |
|        | Session V: Transaction |
| Oct 19 | Serializability<br>[Concurrency Control and Recovery](readings/franklin97concurrency.pdf) |
| Oct 21 | Spanner<br>[Spanner: Google’s Globally-Distributed Database](readings/spanner.pdf) |
| Oct 26 | Student presentation |
| Oct 28 | **Exam 2** |
|        | Session VI: SQL |
| Nov 02 | Query Optimization<br>[Access Path Selection in a Relational Database Management System](readings/selinger.pdf) |
| Nov 04 | [Spanner: Becoming a SQL System](readings/spanner-sql.pdf) |
| Nov 09 | Student presentation |
|        | Session VII: Weaker Isolation and Consistency |
| Nov 11 | More Isolation Levels<br>[A Critique of ANSI SQL Isolation Levels](readings/si.pdf) |
| Nov 16 | MVCC<br>[CockroachDB: The Resilient Geo-Distributed SQL Database](https://doi.org/10.1145/3318464.3386134) |
| Nov 18 | Student presentation |
| Nov 25 | No class (Thanksgiving break) |
| Dec 02 | **Exam 3** |
| Dec 07 | Final project poster presentations |

<!-- Previously in Session VII:
| Nov 11 | MapReduce<br>[Simplified Data Processing on Large Clusters](readings/mapreduce.pdf) |
| Nov 16 | Percolator<br>[Large-scale Incremental Processing Using Distributed Transactions and Notifications](readings/percolator.pdf) |
| Nov 18 | [Velox: Meta's Unified Execution Engine](readings/velox.pdf) |
| Nov 23 | [The Snowflake Elastic Data Warehouse](readings/snowflake.pdf) |
-->

[SBU calendar](https://www.stonybrook.edu/registrar/academic-calendar/current-terms/undergrad-calendar-fall-2026.html)
