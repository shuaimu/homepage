---
layout: page
title: 
permalink: /teaching/se/26fa/
---

<!-- [ -->
<!-- [Home](./index.html)   -->
<!-- [Schedule](./schedule.html)  -->
<!-- | [Policy](./policy.html) -->
<!-- ] -->
# CSE 416-03 (89837) Software Engineering (Fall 2026)

Time: MW 2:00-3:20pm (Aug 24 - Dec 7)

Location: NCS 120

## Announcements

* (Aug 24, 2026) Please join [Piazza](https://piazza.com/stonybrook/fall2026/cse41603) for class notifications and discussions.
<!-- * (Apr 15, 2019) Final exam date: May 20, 2:15pm, Humanities 1003 -->
<!-- * Notes/slides are in this [folder](https://drive.google.com/drive/folders/1imZgiq2q5rvcrOs336eLrBWOLXWCp07o?usp=sharing) (requires SBU login).  -->
<!-- * (Jan 17, 2022) Please read the [collaboration and academic integrity
policy](policy.html); Enrollment in this class suggests you have accepted the
policy. -->

## Quick links (requires SBU login):
* [Piazza](https://piazza.com/stonybrook/fall2026/cse41603)
* [Notes/slides](https://drive.google.com/drive/folders/1imZgiq2q5rvcrOs336eLrBWOLXWCp07o?usp=sharing) 
* [Voluntary Presentation](https://drive.google.com/drive/folders/1ZsE4_GKKfu02G8XZLIav1vYzxvrpRZnu?usp=sharing)
* [Milestone Review](https://drive.google.com/drive/folders/1fappfUmtHpRLG_EL6RjSTqLdpl9Ki5JV?usp=sharing)
    * Note for skipping a milestone review: You don't have to request permission to skip, but it would be better if you notify me in advance. If you want to make up for a missed review, we can try to schedule one if other teams skip their slots. Otherwise, you will need to wait for your next turn. 
    * If you would like to add slides links in the following schedule list, please send pull request to [this page on github](https://github.com/shuaimu/homepage/blob/master/teaching/se/26fa/index.md).


## Course project

You will work in a team on a semester-long software project of your own choosing. Propose something you actually want to build: there is no required domain, architecture, or tech stack. We will carry the project through its complete lifecycle (proposal, design, implementation, testing, and release), checked at six milestones.

AI is encouraged; use it as much as you can.

## Grading

* **Course project (80%)**, delivered in six milestones:
  * Milestone 1: Proposal and Requirements — 10% (due Sep 19)
  * Milestone 2: Design and Setup — 10% (due Oct 3)
  * Milestone 3: MVP — 10% (due Oct 24)
  * Milestone 4: Feature-Complete — 10% (due Nov 7)
  * Milestone 5: Testing and Integration — 10% (due Nov 21)
  * Milestone 6: Final Product — 30% (due Dec 5)
* **Class presentation (20%)**, performed by the group.

Milestone grades consider the quality of what the team delivers and each student's contribution.

## Schedule 

- **Aug 24**: Course Overview (Syllabus)
- **Aug 26**: Lecture: Bitcoin
- **Aug 31**: Lecture: Bitcoin (cont'd)
- **Sep 2**: Lecture: Design Overview
- **Sep 7**: No class (Labor Day)
- **Sep 9**: Presentation: GUI
- **Sep 14**: Milestone 1: Proposal and Requirements
- **Sep 16**: Milestone 1: Proposal and Requirements (due Sep 19)
- **Sep 21**: Workshop
- **Sep 23**: Lecture: DHT
- **Sep 28**: Milestone 2: Design and Setup
- **Sep 30**: Milestone 2: Design and Setup (due Oct 3)
- **Oct 5**: Presentation: Code Review
- **Oct 7**: Lecture: A tour of the Rust Lanugage
- **Oct 12**: No class (Fall Break)
- **Oct 14**: Workshop
- **Oct 19**: Milestone 3: MVP
- **Oct 21**: Milestone 3: MVP (due Oct 24)
- **Oct 26**: Presentation: Testing
- **Oct 28**: Presentation: Debugging
- **Nov 2**: Milestone 4: Feature-Complete
- **Nov 4**: Milestone 4: Feature-Complete (due Nov 7)
- **Nov 9**: Discussion: Boeing
- **Nov 11**: Presentation: "Fake it until you make it"
- **Nov 16**: Milestone 5: Testing and Integration
- **Nov 18**: Milestone 5: Testing and Integration (due Nov 21)
- **Nov 23**: Workshop
- **Nov 25**: No class (Thanksgiving Break)
- **Nov 30**: Milestone 6: Final Demo
- **Dec 2**: Milestone 6: Final Demo (due Dec 5)
- **Dec 7**: Milestone 6: Final Demo (if needed)

* [SBU Academic Calendar](https://www.stonybrook.edu/registrar/academic-calendar/current-terms/undergrad-calendar-fall-2026.html)


## Office hours

MW noon-2pm NCS 351 (Please [book a GCal meeting](https://calendar.app.google/5fEbJJZW86Gcgije6) at least a day before)
